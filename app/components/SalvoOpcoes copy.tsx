...
type SalvoOpcoesProps = {id:string; status:string; doc:Doc};

const SalvoOpcoes: React.FC<SalvoOpcoesProps> = ({id,status, doc}) => {
 ...
 */
  useEffect(() => {
    if (saveSuccess && id) {
        ...
        console.log("rd",rdos)
        const docbr = doc.motivosDePausa? transformPausa(doc.motivosDePausa) : [] as Break[];
        setCurrentBreaks(docbr)
        const doceq = doc.equipamentos? transformEquipamento(doc.equipamentos) : [] as Workforce_Equipment[];
        setCurrentEquipments(doceq);
        const docwf = doc.maoDeObra? transformMaoDeObra(doc.maoDeObra) :[] as Workforce_Equipment[];
        setCurrentWorkforces(docwf)
       }
  }, [saveSuccess, id]);  
    ...
    const findChangedRecords = <T extends { id: string; nome: string } & 
      ...
      const updateAllData = async () => {
        ...
        try {
          ...
          transformMaoDeObra(doc.maoDeObra)
          transformEquipamento(doc.equipamentos)
          transformPausa(doc.motivosDePausa)
...
/*           getInitialWorkforces(DocWorkforces);
      ...
      const transformMaoDeObra = (maoDeObra: Array<{maoDeObra:{nome: string, quantidade?: number} | null}>): Workforce_Equipment[] => {
        return maoDeObra
          .map(item => item.maoDeObra) // Pegando o objeto 'maoDeObra'
          .filter(item => item !== null) // Filtrando valores nulos
          .map(item => {
            const singleW = workforces.find(work => work.nome === item?.nome); // Cruzando com workforces para pegar id
            return singleW ? {
            id: singleW.id,
            nome: item.nome,
      ...
      const transformEquipamento = (equipamentos: Array<{equipamento:{nome: string, quantidade?: number} | null}>): Workforce_Equipment[] => {
        return equipamentos
          .map(item => item.equipamento)
          .filter(item => item !== null)
          .map(item => {
            const singleE = equipments.find(e => e.nome === item?.nome);
            return singleE ? {
            id: singleE.id,
            nome: item.nome,
            quantidade: item.quantidade
      ...
      const transformPausa = (motivosDePausa: Array<{motivoPausa:{nome: string, dataInicio?: string, dataFim?: string} | null}>): Break[] => {
        return motivosDePausa
          .map(item => item.motivoPausa)
          .filter(item => item !== null)
          .map(item => {
            const singleB = breaks.find(b => b.nome === item?.nome);
            return singleB ? {
            id: singleB.id,
            nome: item.nome,
            dataHoraInicio: item.dataInicio,
            dataHoraFim: item.dataFim
          } : null;
        })
          .filter(item => item !== null);
      };
  ...
  return (
    <View style={{ padding: 5, borderTopWidth: 1, marginTop: 10 }}>
      ...
      {(rdos?.maoDeObra.length !== null && rdos?.maoDeObra.length !== 0) && (
        <View style={{ padding: 10, borderWidth: 1, borderRadius: 5, marginTop: 10 }}>
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 20 }}>
      ...
      {(rdos?.equipamentos.length !== null && rdos?.equipamentos.length !== 0) && (
          <View style={{ padding: 10, borderWidth: 1, borderRadius: 5, marginTop: 10 }}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 20 }}>
      ...
      {(rdos?.motivosDePausa.length !== null && rdos?.motivosDePausa.length !== 0)&& (
          <View style={{ padding: 10, borderWidth: 1, borderRadius: 5, marginTop: 10 }}>
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 20 }}>
